...
VERSION = "v9.8.2"
SUPPORTED_PREVIOUS_VERSIONS = ("v9.8.0", "v9.8.1")
FILES = (
...
def _smoke(project_dir: Path) -> None:
    ...
    try:
        ...
        assert fetched_options[0]["timeout"] == 8
        assert fetched_options[0]["retries"] == 0
        assert any(row.get("stage") == "nps_complete" for row in events)

        center_source = (project_dir / "prospect_db_center.py").read_text(
            encoding="utf-8"
        )
        assert "약 2분 안에 검색을 종료" in center_source
        assert "네이버 공개검색" in center_source
        assert "time_limit_seconds=120" in center_source
    finally:
...
def main() -> int:
    ...
    backup_dir = project_dir / "_update_backups" / "before_v9.8.2"
    ...
    print("NPS_TIMEOUT=8_SECONDS_NO_RETRY")
    print("SEARCH_LIMIT=120_SECONDS_PARTIAL_RESULTS")
    print("OFFICIAL_WEBSITE_SCAN=BOUNDED")
