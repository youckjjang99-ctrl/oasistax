-- OASIS CRM v10.2.3 - 경정청구 문서별 저장 결과 기록
-- 공개 테이블 직접 쓰기는 계속 차단하고 Railway service_role 전용 RPC만 허용한다.
...
begin
    ...
    if v_status = 'ready' then
        ...
        v_facts := v_facts || jsonb_build_object(
            'safe_error_code',
            left(coalesce(p_safe_error_code, 'DOCUMENT_COLLECTION_FAILED'), 80)
        );
    end if;
