...
def append_user_customer_db(uploaded_excel_path, user_id, manager_name=""):
    ...
    try:
        ...
        df_all = pd.concat([df_old, df_new], ignore_index=True, sort=False) if not df_old.empty else df_new
...
def append_cretop_to_user_customer_db(pdf_path, user_id, manager_name="", duplicate_action="skip"):
    ...
    is_dup = False
    if not df_old.empty and business_no and "사업자등록번호" in df_old.columns:
        is_dup = df_old["사업자등록번호"].apply(normalize_business_no).eq(business_no).any()

    if is_dup and duplicate_action == "skip":
        return cumulative_path, 0, "이미 같은 사업자등록번호가 누적DB에 있습니다. 저장하지 않았습니다.", data, df_new

    if is_dup and duplicate_action == "update":
        mask = df_old["사업자등록번호"].apply(normalize_business_no).eq(business_no)
        df_old = df_old.loc[~mask].copy()
        df_all = pd.concat([df_old, df_new], ignore_index=True, sort=False)
        message = "기존 동일 사업자번호 데이터를 최신 크레탑 추출값으로 업데이트했습니다."
    else:
        df_all = pd.concat([df_old, df_new], ignore_index=True, sort=False) if not df_old.empty else df_new
        message = "크레탑 PDF 추출값을 내 누적 고객DB에 추가했습니다."

    _write_cumulative_customer_db(cumulative_path, df_all, columns)

    return cumulative_path, 1, message, data, df_new
