...
def _smoke(project_dir: Path) -> None:
    ...
    try:
        ...
        sql_source = (
            project_dir / "supabase_v985_employee_snapshots.sql"
        ).read_text(encoding="utf-8").lower()
        assert "oasis_nps_employee_snapshots" in sql_source
        assert "enable row level security" in sql_source
...
def main() -> int:
    ...
    if current_version and current_version not in SUPPORTED_PREVIOUS_VERSIONS:
        ...
        raise RuntimeError(
            f"v9.8.4에서 실행해 주세요. 현재={current_version}, "
            f"지원={allowed}"
        )

    backup_dir = project_dir / "_update_backups" / "before_v9.8.5"
