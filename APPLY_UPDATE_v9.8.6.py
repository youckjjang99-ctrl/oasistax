...
VERSION = "v9.8.6"
SUPPORTED_PREVIOUS_VERSIONS = ("v9.8.5",)
FILES = (
...
def _smoke(project_dir: Path) -> None:
    ...
    try:
        ...
        assert "def load_prior_employee_snapshots" in repository_source
        assert "def save_employee_snapshots" in repository_source
        assert "def _snapshot_rows" in repository_source
        assert "company_address_keys" in repository_source
        assert "월별 가입자 스냅샷 저장을 완료하지 못했습니다" in center_source
        ...
        rows = prospect_db_repository._snapshot_rows([first, same, branch])
        assert len(rows) == 2
        assert prospect_db_repository._snapshot_identity(first) != (
            prospect_db_repository._snapshot_identity(branch)
        )
        kept, excluded = prospect_db_repository.remove_existing_prospects(
            [first, branch],
            source_keys=set(),
            business_nos=set(),
            company_address_keys={
                prospect_db_repository._company_address_key(
                    first["사업장명"], first["주소"]
                )
            },
        )
        assert excluded == 1 and kept == [branch]
...
def main() -> int:
    ...
    if current_version and current_version not in SUPPORTED_PREVIOUS_VERSIONS:
        ...
        raise RuntimeError(
            f"v9.8.5에서 실행해 주세요. 현재={current_version}, "
            f"지원={allowed}"
        )

    backup_dir = project_dir / "_update_backups" / "before_v9.8.6"
