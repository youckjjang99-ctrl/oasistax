...
- 카카오톡 버튼은 Solapi의 승인된 `경정청구 자료수집 인증안내` 알림톡만
...
본문을 자유 작성하지 않으며 승인된 `경정청구 자료수집 인증안내` 템플릿의
`#{고객명}`, `#{인증링크}` 변수만 전달합니다. 템플릿 자체에 HTTPS가 포함되어
있으므로 인증링크에는 `http://` 또는 `https://`를 입력하지 않습니다.
