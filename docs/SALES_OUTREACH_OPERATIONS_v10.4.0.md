...
- 저장된 영업후보 표는 업체명, 사업자번호, 사업자유형, 연락처, 이메일,
  ...
  표시합니다.
- 이메일·문자 버튼은 한 업체만 대상으로 하는 자유작성 창을 엽니다.
- 카카오톡 버튼은 운영 환경에 등록된 경정청구 알림톡 5종 중 하나를 선택해
  사용하며, 담당자는 `고객이름`과 스킴을 제외한 `인증링크`만 입력합니다.
- 수신자가 다른 번호로 발송을 요청한 경우 발송창에서 휴대폰 번호를 바꿀 수
  있습니다. 이 값은 해당 발송에만 사용하며 업체 DB의 저장 연락처를 변경하거나
  outbox에 원문으로 저장하지 않습니다.
...
자동으로 붙입니다. 카카오톡은 승인된 Solapi 알림톡 템플릿을 사용하고 SMS
대체 발송은 하지 않습니다. 문자·카카오톡 발송은 한국시간 08:00 이상, 21:00 미만에만
허용합니다.
...
저장된 영업후보의 카카오톡 버튼은 Solapi 알림톡 API를 사용합니다. 화면에서
본문을 자유 작성하지 않으며 승인된 경정청구 알림톡 템플릿의 `#{고객명}`,
`#{인증링크}` 변수만 전달합니다. 템플릿 자체에 HTTPS가 포함되어 있으므로
인증링크에는 `http://` 또는 `https://`를 입력하지 않습니다. 화면에서 바꾼
휴대폰 번호는 형식과 수신거부 상태를 다시 확인하고, 실제 발송번호의 HMAC만
중복방지 outbox에 저장합니다.

- `SOLAPI_API_KEY`
- `SOLAPI_API_SECRET`
- `SOLAPI_KAKAO_CHANNEL_ID`
- `SOLAPI_TEMPLATE_AUTH_START_ID`: `경정청구 자료수집 인증안내` 승인 템플릿 ID
- `SOLAPI_TEMPLATE_AUTH_RESUME_ID`: 자료수집 재안내 승인 템플릿 ID
- `SOLAPI_TEMPLATE_NEXT_AUTH_ID`: 추가 인증 안내 승인 템플릿 ID
- `SOLAPI_TEMPLATE_COMPLETE_ID`: 자료수집 완료 안내 승인 템플릿 ID
- `SOLAPI_TEMPLATE_FAILED_ID`: 자료수집 확인 필요 안내 승인 템플릿 ID

문자 대체발송은 비활성화하며, 현재 담당자 배정·정규 휴대전화·수신거부 상태를
발송 직전에 다시 확인합니다.
