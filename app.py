...
if active_tab == "홈":
    ...
    st.markdown("## 기업정보등록")
    st.caption("기업을 먼저 등록한 뒤, 필요한 자료만 선택해 연결하세요.")
    registration_route = st.radio(
        "무엇을 하시겠어요?",
        ["새 기업 등록", "등록기업에 자료 추가"],
        horizontal=True,
        key="enterprise_registration_route",
    )
    if registration_route == "등록기업에 자료 추가":
        render_enterprise_information_assets(CURRENT_USER_ID, CURRENT_USER_NAME)
    ...
    if registration_type.startswith("개인사업자"):
        render_personal_business_registration(
            CURRENT_USER_ID, CURRENT_USER_NAME, USER_UPLOAD_DIR,
        )
        _finish_enterprise_menu_transition(ENTERPRISE_TRANSITION_PLACEHOLDER)
        st.stop()

    render_cretop_registration(CURRENT_USER_ID, CURRENT_USER_NAME, USER_UPLOAD_DIR)
