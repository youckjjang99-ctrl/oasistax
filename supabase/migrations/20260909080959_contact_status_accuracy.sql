...
create index if not exists oasis_company_sales_contact_logs_assignment_latest_idx
on public.oasis_company_sales_contact_logs (
    assignment_id,
    contacted_at desc,
    created_at desc,
    id desc
...
create or replace function public.oasis_get_user_db_dashboard(
...
returns table (
    ...
    new_db_count bigint,
    in_progress_db_count bigint,
    completed_db_count bigint
)
language plpgsql
stable
security invoker
...
declare
    ...
    v_is_admin boolean := public.oasis_sales_actor_is_admin(v_user_id);
begin
    ...
    with owned as (
        select
            ...
            latest.contacted_at as latest_contacted_at,
        ...
        left join lateral (
            ...
            where l.assignment_id = a.id
              -- The row ID is reused on reassignment; never borrow a previous
              -- owner's activity, even when its contacted_at was backdated.
              and (
                  coalesce(a.current_assignment_contact_count, 0) > 0
                  or coalesce(a.legacy_hold, false)
              )
              and l.assigned_user_id = a.assigned_user_id
              and (v_is_admin or l.created_by_user_id = v_user_id)
              and (
                  coalesce(a.legacy_hold, false)
                  or a.assigned_at is null
                  or l.created_at >= a.assigned_at
              )
            order by
                l.contacted_at desc,
                l.created_at desc,
    ...
    select
        ...
        count(*) filter (where db_stage = 'new')::bigint,
        count(*) filter (where db_stage = 'in_progress')::bigint,
        count(*) filter (where db_stage = 'completed')::bigint
    from classified;
end;
$$;

create or replace function public.oasis_list_user_db_assignments_v2(
    p_current_user_id text,
...
returns table (
    ...
    total_count bigint,
    latest_contact_result text,
    latest_contacted_at timestamptz,
    latest_next_contact_at timestamptz,
    current_assignment_contact_count integer,
    contact_summary_loaded boolean
)
language plpgsql
stable
security invoker
...
begin
    ...
    select
        ...
        filtered.updated_at,
        count(*) over ()::bigint,
        filtered.latest_contact_result,
        filtered.latest_contacted_at,
        filtered.latest_next_contact_at,
        coalesce(filtered.current_assignment_contact_count, 0),
        true
    from filtered
    order by
...
revoke execute on function public.oasis_list_user_db_assignments_v2(
    text, text, integer, integer
) from public, anon, authenticated;
grant execute on function public.oasis_list_user_db_assignments_v2(
    text, text, integer, integer
) to service_role;

-- Keep the legacy list RPC intact for older deployments.
revoke execute on function public.oasis_get_user_db_dashboard(text)
from public, anon, authenticated;
grant execute on function public.oasis_get_user_db_dashboard(text)
to service_role;
