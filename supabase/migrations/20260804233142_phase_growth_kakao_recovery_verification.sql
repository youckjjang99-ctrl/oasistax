...
alter table oasis_private.oasis_growth_kakao_recovery_runs
    add column verified_count integer not null default 0
        check (verified_count >= 0);

alter table oasis_private.oasis_growth_kakao_recovery_runs
    add constraint oasis_growth_kakao_recovery_runs_verify_order_check
    check (verified_count <= recovered_count);
...
create index oasis_growth_kakao_verification_pending_idx
    on oasis_private.oasis_growth_kakao_recovery_items (
        run_id,
        contact_key
    )
    where recovered_at is not null and verified_at is null;
        ...
        (
            select jsonb_build_object(
                ...
                'pending_verification_count',
                    r.recovered_count - r.verified_count
            )
