...
VERSION = "v9.7.5"
SUPPORTED_PREVIOUS_VERSIONS = ("v9.7.1", "v9.7.2", "v9.7.3", "v9.7.4")
FILES = (
...
def _smoke(project_dir: Path) -> None:
    ...
    try:
        ...
        assert "contact_ready_items" in source
        assert "contact_missing_count" in source
        assert "대표전화가 확인되지 않은 업체" in source
...
def main() -> int:
    ...
    backup_dir = project_dir / "_update_backups" / "before_v9.7.5"
