...
VERSION = "v9.8.8"
SUPPORTED_PREVIOUS_VERSIONS = ("v9.8.7",)
FILES = (
...
def _smoke(project_dir: Path) -> None:
    ...
    try:
        ...
        assert public_data_api.REGION_CODES["제주특별자치도"] == "50"
        assert public_data_api.REGION_CODES["부산광역시"] == "26"
        assert public_data_api._region_from_address(
            "제주특별자치도 제주시", "50"
        ) == "제주특별자치도"
        assert public_data_api._region_from_address(
            "경상남도 창원시", "48"
        ...
        combined_row = {
            "source_key": "period-seq",
            "가입자수": 12,
            "자료생성년월": "202607",
            "원본데이터": {"seq": "period-seq"},
        }
        with (
            patch.dict(
                public_data_api.os.environ,
            ...
            combined_rows, combined_stats = (
                public_data_api.enrich_employment_growth([combined_row])
            )
        assert combined_stats["employment_checked"] == 1
        assert combined_rows[0]["순고용증가"] == 3
        assert combined_rows[0]["고용증가신호"] is True
        assert combined_rows[0]["전년동월상태"] == "스냅샷 축적 전"
        ...
        assert result["growth_basis"] == "combined"
        assert result["items"][0]["순고용증가"] == 2
        assert result["snapshot_warning"] == ""
        ...
        assert "시·군·구 법정동 코드" not in center_source.split(
            "def render_prospect_db_center", 1
        )[1]
        assert "읍·면·동 법정동 코드" not in center_source.split(
            "def render_prospect_db_center", 1
        )[1]
        ...
        assert "def format_establishment_years" in app_source
        assert "**설립연차**" in app_source
        assert "신규기업" in app_source

        assert "전국 주식회사 수집" in center_source
        ...
        with patch.object(
            ...
            assert prospect_db_repository.list_prospects("user-a") == []
        assert captured.get("filters") == {"owner_user_id": "user-a"}
        assert prospect_db_repository.list_prospects("") == []
        repository_source = (
            project_dir / "prospect_db_repository.py"
        ).read_text(encoding="utf-8")
        assert 'filters={"owner_user_id": owner_user_id}' in repository_source
        assert '"owner_user_id": f"eq.{owner_user_id}"' in repository_source
        assert "내가 저장한 영업후보만 표시합니다" in center_source
...
def main() -> int:
    ...
    if current_version and current_version not in SUPPORTED_PREVIOUS_VERSIONS:
        ...
        raise RuntimeError(
            f"v9.8.7에서 실행해 주세요. 현재={current_version}, "
            f"지원={allowed}"
        )

    backup_dir = project_dir / "_update_backups" / "before_v9.8.8"
    ...
    print("EMPLOYMENT_BASIS=COMBINED_SIGNAL")
    print("YEAR_OVER_YEAR=NOT_A_HARD_FILTER")
    print("YEAR_OVER_YEAR_SNAPSHOT=ENABLED")
    print("SNAPSHOT_BATCH_DEDUPLICATION=ENABLED")
    print("PERSONAL_BUSINESS_NAME_ADDRESS_EXCLUSION=ENABLED")
    print("NATIONWIDE_REGION_SELECTION=ENABLED")
    print("ESTABLISHMENT_YEARS_DISPLAY=ENABLED")
    print("SAVED_PROSPECT_LIST=OWNER_ONLY")
    print("GLOBAL_DUPLICATE_EXCLUSION=ALL_USERS")
    print("LEGAL_DONG_CODE_INPUT=REMOVED")
